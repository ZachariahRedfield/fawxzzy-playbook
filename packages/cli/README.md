...
- `playbook init`
- `playbook analyze`
- `playbook index` (writes `.playbook/repo-index.json` for machine-readable repository intelligence)
- `playbook verify`
- `playbook doctor`
- `playbook diagram`
...
Example `repo-index.json` shape generated by `playbook index`:
...
```json
{
  ...
  "modules": ["api", "ui"],
  "database": "supabase",
  "rules": ["requireNotesOnChanges", "verify.rule.tests.required"]
}
```
