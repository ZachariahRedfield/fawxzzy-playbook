...
const readPatternCandidate = (playbookHome: string, candidateId: string): Record<string, unknown> => {
  const artifact = readJsonIfPresent<PatternCandidatesArtifact>(path.join(playbookHome, '.playbook', 'pattern-candidates.json'));
  const candidate = artifact?.candidates?.find((entry) => String(entry.id ?? '') === candidateId);
  if (!candidate) throw new Error(`playbook promote: pattern candidate not found: ${candidateId}`);
  return candidate;
};

const parseSourceRef = (sourceRef: PromotionSourceRef): { kind: 'story' | 'pattern'; candidateId: string; repoId?: string } => {
  ...
  if (globalMatch) return { kind: 'pattern', candidateId: globalMatch[1] };
  ...
  try {
    if (target === 'story') {
      ...
      const storyCandidate = parsedSource.kind === 'story'
        ...
        : null;
      const patternCandidate = parsedSource.kind === 'pattern'
        ? readPatternCandidate(playbookHome, parsedSource.candidateId)
        : null;
    ...
    const patternCandidate = readPatternCandidate(playbookHome, parsedSource.candidateId);
