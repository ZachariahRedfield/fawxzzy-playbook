...
export const runBootstrapProof = (
  ...
  const cliResolutionAttempts = pnpmCheck.ok
    ? [
        { label: 'pnpm exec playbook --version', result: runCommand('pnpm', ['exec', 'playbook', '--version'], repoRoot) },
        { label: 'pnpm playbook --version', result: runCommand('pnpm', ['playbook', '--version'], repoRoot) }
      ]
    : [];
  const successfulCliAttempt = cliResolutionAttempts.find((attempt) => attempt.result.ok) ?? null;
  checks.push(buildCheck({
    id: 'cli.playbook-resolution',
    stage: 'cli',
    status: successfulCliAttempt ? 'pass' : 'fail',
    category: successfulCliAttempt ? null : 'binary_resolution_failed',
    summary: successfulCliAttempt ? `Local Playbook CLI resolves via ${successfulCliAttempt.label}.` : 'Local Playbook CLI resolution failed.',
    diagnostics: successfulCliAttempt
      ? [successfulCliAttempt.result.stdout.trim() || `${successfulCliAttempt.label} succeeded`]
      : [
          pnpmCheck.ok ? null : 'skipped because pnpm runtime is unavailable',
          ...cliResolutionAttempts.flatMap((attempt) => {
            const diagnostics = [attempt.result.errorMessage, attempt.result.stderr.trim()].filter(
              (value): value is string => typeof value === 'string' && value.length > 0
            );
            return diagnostics.length > 0 ? diagnostics.map((value) => `${attempt.label}: ${value}`) : [`${attempt.label}: command returned non-zero exit status`];
          })
        ].filter((value): value is string => typeof value === 'string' && value.length > 0),
    next_action: successfulCliAttempt ? null : 'Ensure this repository can resolve Playbook through `pnpm exec playbook --version` or `pnpm playbook --version`.',
    command: successfulCliAttempt?.label ?? 'pnpm exec playbook --version'
  }));
