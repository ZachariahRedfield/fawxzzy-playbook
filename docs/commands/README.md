...
| `interop` | Register capabilities, emit bounded remediation requests, run mock lifeline adapter cycles, and inspect request/receipt/health state | canonical | remediation | secondary | Later | Current (implemented) | `pnpm playbook interop inspect --surface all --json` |
