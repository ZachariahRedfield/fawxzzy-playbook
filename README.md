...
Recent implementation note: repository CI now captures failed `pnpm test` output to `.playbook/ci-failure.log`, optionally runs the canonical `pnpm playbook test-autofix --input .playbook/ci-failure.log --json` and `pnpm playbook remediation-status --json` commands behind explicit fail-closed mutation gates, uploads the resulting `.playbook/` remediation artifacts, and renders one sticky PR remediation comment from those artifacts without introducing workflow-local repair logic.
